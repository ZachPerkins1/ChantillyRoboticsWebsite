...
import comm_manager as cmanager
...
app.config['ADDRESS'] = "http://website-thezperk.c9users.io"
...
app.config['MAIL_USERNAME'] = "chantillyrobotics.612.org@gmail.com"
...
dmanager.init(r)
cmanager.init(mail, app.config.get('MAIL_USERNAME'), app.config.get('ADDRESS'))
sess.init(r)
add_tags()
...
def register():
    ...
    if request.method == "GET":
        ...
        if errs.any():
            resp = make_response(render_template("admin/register.html", errors=errs.get_formatted(), cached_data=request.form, reg_code=code))
        else:
...
def user_manager():
    def format_user_list():
        user_index = sess.get_all_usernames()
    ...
    if request.method == "GET":
        ...
        email = cmanager.Email("Registration", "registration.html")
        email.add("reg_code", registration.get_reg_code())
        cmanager.send_email([data['email']], email)
